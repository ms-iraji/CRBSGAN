...
parser.add_argument('--dataset', metavar='DATASET', default='cifar10',
                        choices=['cifar10','svhn','mnist','cinic','tiny','stl10'],
...
def train(trainloader,unlabelledloader, model, ema_model, optimizer, epoch, filep):
    ...
    for o,(u, _) in enumerate(unlabelledloader):
        ...
        C_fake_wei = torch.zeros(C_fake_wei.size(0), 10).cuda().scatter_(1, C_fake_wei, 1)
        ...
        loss = 0.1* C_fake_loss+ mixup_consistency_loss+class_loss#+0.1*C_fake_lossmix+C_labeled_loss +C_unlabeled_loss+generated_weight(epoch)*C_u_lossmix
        meters.update('loss', loss.item())
        ...
        if True:        
                ...
                #actl=D.features
                #consistency_relation_distl = torch.sum(relation.losses2.relation_mse_loss(acl, actl)) / D_labeledt.size(0)
                ...
                #actul=D.features
                D_fake_loss = BCEloss(D_fake, torch.zeros_like(D_fake)  )
                #consistency_relation_distul = torch.sum(relation.losses2.relation_mse_loss(acul, actul)) / D_unlabeledt.size(0)
                ...
                #consistency_relation_distdf = torch.sum(relation.losses2.relation_mse_loss(actdgtz, actdf)) / D_G_T_z.size(0)

                D_loss = D_labeled_loss + D_unlabeled_loss + D_fake_loss+((L_reall1+L_realu2+L_D_fake1)*10)+L_dis*5#+consistency_relation_distl*0.1+consistency_relation_distul*0.1+consistency_relation_distdf*0.1#self.train_hist['D_loss'].append(D_loss.item())
        ...
        if True:
                ...
                #consistency_relation_distfg = -torch.sum(relation.losses2.relation_mse_loss(G_, G_T_z)) / G_.size(0)
                ...
                G_loss = G_loss_D + G_loss_C+L_gen*0.5#+consistency_relation_distfg*0.1
